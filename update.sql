...
INSERT INTO catalog_relations
    SELECT version, entity_name, entity_kind, attributes, system_attributes
    FROM current_version_delta
    WHERE source ~ 'current'
;

DELETE FROM catalog_relations r
    USING current_version_delta d
    WHERE r.version = d.version
        AND r.entity_name = d.entity_name
        AND d.source ~ 'stored'
;
COMMIT;
